...
export class PreferencesDialog implements OnInit, OnChanges{
    ...
    cancel() {
        this.snackbar.open('Preferences not saved', null, {duration: 6000, horizontalPosition: "start", panelClass: ['mat-elevation-z2']});
        this.dialogRef.close();
    }
    close() {
        this.shared.setSettings(this.settings);
        this.dialogRef.close();
        let snackbarRef = this.snackbar.open('Preferences saved', 'Reload', {duration: 7000, horizontalPosition: "start", panelClass: ['mat-elevation-z2']});
